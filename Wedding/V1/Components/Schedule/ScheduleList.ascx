<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ScheduleList.ascx.cs" Inherits="V1_Components_Schedule_ScheduleList" %>
    ...
    <table width="100%" border="0" cellpadding="1px" cellspacing="1px">
        ...
        <tr class="tb-header2">
            <td colspan="2" style="padding: 3px 3px;">
                <input type="button" value="刪除" onclick="DelectSchedule();" />&nbsp;|&nbsp;<input type="button" value="导出" disabled="disabled" />
            </td>
        </tr>
        <tr>
        ...
        <asp:Repeater ID="rpList" runat="server">
            <HeaderTemplate>
                ...
                <td class="td-2"><input type="checkbox" onclick="CheckSelects(this,'dv-guest-search-content');"; /></td>
                    <td class="td-2">
                        <%=GetSortHeader("英文名", "ScheduleName_en_us")%>
                    </td>
                    <td class="td-2">
                        <%=GetSortHeader("中文名", "ScheduleName_zh_tw")%>
                    </td>
                    <td class="td-2">
                    ...
                    <td class="td-2">
                        <%=GetSortHeader("占席", "ScheduleQuantity")%>
                    </td>
                    <td class="td-2">
                        <%=GetSortHeader("郵箱", "ScheduleEmail")%>
                    </td>
                    <td class="td-2">
                        <%=GetSortHeader("電話", "ScheduleMobile")%>
                    </td>
                    <td class="td-2">
                        <%=GetSortHeader("已出席", "ScheduleAttended")%>
                    </td>
                    <td class="td-2">
            ...
            <ItemTemplate>
                <tr id="tr-guest-<%#Eval("ScheduleCode") %>"><td><span class="num"><%#Container.ItemIndex+1+(PageSize*(PageIndex-1)) %></span></td>
